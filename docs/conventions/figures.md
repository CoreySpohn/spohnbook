...
The chapters open their scientific explanations with physical overview diagrams: the objects, planes and measurements a reader should be able to point at before the first equation. Each comes from one module in `tools/explainers/`, which renders a light and a dark documentation still (PNG for the page, PDF as the vector export), a 16:9 slide still in `talks/stills/`, and, where motion teaches the mechanism, light and dark documentation videos in `docs/_static/explainers/` plus a talk video in `talks/animations/`. A few clips that show one object being transformed are rendered with Manim (the optional `explainers-manim` extra; `--no-manim` keeps the committed clips); each has a matplotlib still that documents it. Speaker notes are in `talks/notes/`.
...
| Observer construction, three-rotation strip, orbit elements, phase epochs; three-rotation Manim clip, viewpoint and orbit videos | Where are the angles measured, from which axis and in which direction, and how does the orbit become the observed offset? | [Geometry and time](geometry-time.md) | `tools/explainers/d02_orbit_geometry.py` |
| Where each collection factor lives, radiometric collection, and the four-pixel reference | Which operation turns a density over wavelength, solid angle and area into the count in one pixel? | [Radiometry](radiometry-detectors.md), [reference case](../examples/photon-to-electron-reference.md) | `tools/explainers/d03_radiometric_collection.py` |
| Optical planes, the light at every plane, and an OPD perturbation; plane-tour video | Where do fields and images live, and which steps keep complex amplitudes versus produce an intensity? | [Optics](optics-images.md) | `tools/explainers/d04_optical_planes.py` |
| Lenslet IFS instrument, the three origins, PSFlet placement and extraction; wavelength-scan video | How does one lenslet's light become a trace on the detector, and why are neighboring extracted bins correlated? | [Optics](optics-images.md) | `tools/explainers/d05_lenslet_ifs.py` |
...
| Information boundary, record under a reporting law, campaign timeline; fit and forecast timeline | Which objects exist in the simulated world, which are measured, and which are inferred? | [Measurements and records](inference-records.md), [fit and forecast](../examples/fit-and-forecast.md) | `tools/explainers/d08_experiment_record_model.py` |
| Roll is not rotation; roll video | Does telescope roll move the image content, or the detector under a fixed sky? | [Optics](optics-images.md) | `tools/explainers/d09_roll_rotation.py` |
| Surface, OPD, phase and phasor | How does a mirror displacement become a signed intensity change? | [Optics](optics-images.md) | `tools/explainers/d10_phasor.py` |
| Barycenter, reflex motion and the sign of RV; reflex clock video | Which body moves where about the barycenter, and what sign does a receding star have? | [Geometry and time](geometry-time.md) | `tools/explainers/d11_barycenter_rv.py` |
| Which contrast denominator | Which denominator does a quoted contrast use, and how much do they differ for one planet? | [Radiometry](radiometry-detectors.md) | `tools/explainers/d12_contrast_denominators.py` |
| Fixed versus native angular grids | On which grid does a source stay put, and on which does diffraction? | [Optics](optics-images.md) | `tools/explainers/d13_angular_grids.py` |
